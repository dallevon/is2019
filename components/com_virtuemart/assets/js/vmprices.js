...
Virtuemart.product = function(carts) {
	carts.each(function() {
		...
		quantity
			.off('click blur submit', Virtuemart.checkQuantity)
			.on('click blur submit', Virtuemart.checkQuantity)
...
Virtuemart.checkQuantity = function(e) {
	e.preventDefault();
	...
	if (['submit', 'text', 'number'].indexOf(eventTargetType) !== -1) {
		...
		const cartForm = $eventTarget.closest('form');

		if (cartForm.length) {
			formErrorMessage = cartForm.data().errorMessage;
			if (eventTargetType === 'submit') {
				$inputField = cartForm.find('input[name="quantity[]"]');
			}
		}

		step = parseInt($inputField.attr('step'), 10);
		step = isNaN(step) ? 1 : step;

		quantity = parseInt($inputField.val(), 10);
		if (isNaN(quantity) || quantity < 1) {
			$inputField.val(step);
			return false;
		}

		max = parseInt($inputField.attr('max'), 10);
		max = isNaN(max) ? 1 : max;

		if (eventTargetType === 'submit' && quantity > max) {
			if (formErrorMessage) {
				alert(formErrorMessage.replace('%s', max));
				$inputField.val(max);
			}
			return false;
		}

		fieldErrorMessage = $inputField.data().errStr;
		const remainder = quantity % step;

		if (remainder !== 0) {
			if (!isNaN(step) && fieldErrorMessage) {
				alert(fieldErrorMessage.replace('%s', step));
			}
			if (quantity > remainder) {
				$inputField.val(quantity - remainder);
			} else {
				$inputField.val(step);
			}
			return false;
