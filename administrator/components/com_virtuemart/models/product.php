...
class VirtueMartModelProduct extends VmModel
{
    ...
    public static $decimals = array('product_length','product_width','product_height','product_weight','product_packaging');
    public $_onlyQuery 	= false;
    ...
    public $keyword = "0";
    public $product_parent_id = false;
    public $virtuemart_manufacturer_id = false;
    public $virtuemart_category_id = 0;
    public $search_type = '';
    public $searchcustoms = false;
    public $searchplugin = 0;
    public $filter_order = 'p.virtuemart_product_id';
    public $filter_order_Dir = 'DESC';
    public $valid_BE_search_fields = array('product_name', '`p`.product_sku','`l`.`slug`', 'product_s_desc', '`l`.`metadesc`');
    private $_autoOrder = 0;
    private $orderByString = 0;
    private $listing = false;


    public static function getValidProductFilterArray()
    {
        static $filterArray;
     ...
     * @author Max Milbers
     */
    public function initialiseRequests()
    {
        $this->keyword = false;
    ...
    protected function populateState()
    {
        ...
        if ($app->isSite() and !vRequest::getInt('manage', false)) {
            ...
            if (!empty($this->searchcustoms)) {
                if (is_object($this->searchcustoms)) {
                    $this->searchcustoms = get_object_vars($this->searchcustoms);
                }
            ...
            if (!empty($this->virtuemart_custom_id)) {
                if (is_object($this->virtuemart_custom_id)) {
                    $this->virtuemart_custom_id = get_object_vars($this->virtuemart_customfield_id);
                }
    ...
    public function sortSearchListQuery($onlyPublished = true, $virtuemart_category_id = false, $group = false, $nbrReturnProducts = false, $langFields = array())
    {
        ...
        if (!empty($this->keyword) and $group === false) {
            ...
            foreach ($this->valid_search_fields as $searchField) {
                ...
                if ($searchField == 'category_name' || $searchField == 'category_description') {
                    ...
                    $joinPrice = true;
                } elseif ($searchField == 'product_name' or $searchField == 'product_s_desc' or $searchField == 'product_desc' or $searchField == 'slug') {
        ...
        if ($isSite and !VmConfig::get('use_as_catalog', 0)) {
            if (VmConfig::get('stockhandle_products', false)) {
                ...
                if (($product_stockhandle->disableit_children || VmConfig::get('stockhandle', 'none') == "disableit_children") && ($product_stockhandle->disableit || VmConfig::get('stockhandle', 'none') == "disableit")) {
                    $where[] = ' CASE
								 ...
								 END = 1 ';
                    $joinChildren = true;
                } elseif ($product_stockhandle->disableit_children || VmConfig::get('stockhandle', 'none') == "disableit_children") {
                    $where[] = ' CASE
									WHEN (p.`product_stockhandle` = "0" AND "'. VmConfig::get('stockhandle', 'none') .'" = "disableit_children") OR (p.`product_stockhandle` = "disableit_children")
										THEN ((p.`product_in_stock` - p.`product_ordered`) >"0" OR (children.`product_in_stock` - children.`product_ordered`) > "0")
									ELSE 1
								 END = 1 ';
                    $joinChildren = true;
                } elseif ($product_stockhandle->disableit || VmConfig::get('stockhandle', 'none') == "disableit") {
                    $where[] = ' CASE
									WHEN (p.`product_stockhandle` = "0" AND "'. VmConfig::get('stockhandle', 'none') .'" = "disableit") OR (p.`product_stockhandle` = "disableit")
										THEN p.`product_in_stock` - p.`product_ordered` > "0"
									ELSE 1
            ...
            } elseif (VmConfig::get('stockhandle', 'none') == "disableit_children") {
        ...
        vmdebug('my filter order ', $this->filter_order);
    ...
    public static function getCurrentUserShopperGrps()
    {
        static $ids = false;
    ...
    public static function checkIfCached($virtuemart_product_id, $front = null, $withCalc = true, $onlyPublished = true, $quantity = 1, $virtuemart_shoppergroup_ids = 0, $withRating = 0)
    {
        ...
        } elseif (empty($withCalc) or empty($withRating)) {
    ...
    public static $_products = array();
    public static $_cacheOpt = array();
    public static $_cacheOptSingle = array();
    public static $_alreadyLoadedIds = array();
    public static $omitLoaded = false;
    ...
    public function getProduct($virtuemart_product_id = null, $front = true, $withCalc = true, $onlyPublished = true, $quantity = 1, $virtuemart_shoppergroup_ids = 0)
    {
        ...
        if ($checkedProductKey[0]) {
            if (self::$_products[$checkedProductKey[1]]===false) {
                return false;
            } elseif (is_object(self::$_products[$checkedProductKey[1]])) {
        ...
        while (!empty($child->product_parent_id)) {
            ...
            foreach ($attribs as $k=> $v) {
                if (!property_exists($parentProduct, $k)) {
                    continue;
                }
        ...
        if ($front and $stockhandle == 'disableit' and ($child->product_in_stock - $child->product_ordered) <= 0) {
            ...
            if (($child->product_in_stock - $child->product_ordered) < 1) {
                ...
                } elseif ($stockhandle == 'risetime' and VmConfig::get('rised_availability') and empty($child->product_availability)) {
                ...
                } elseif (!empty($child->product_availability)) {
            ...
            } elseif ($product_available_date != '0000-00-00' and $current_date < $product_available_date) {
                $child->availability = vmText::_('COM_VIRTUEMART_PRODUCT_AVAILABLE_DATE') .': '. JHtml::_('date', $child->product_available_date, vmText::_('DATE_FORMAT_LC4'));
            }
        ...
        if (!empty($product->allPrices) and is_array($product->allPrices)) {
            ...
            foreach ($product->allPrices as $k=>$price) {
                ...
                if (empty($price['price_quantity_end']) and $price['price_quantity_start'] <= $quantity) {
                    $quantityFits = true;
                } elseif ($price['price_quantity_start'] <= $quantity and $quantity <= $price['price_quantity_end']) {
                    $quantityFits = true;
                } else {
                    $quantityFits = false;
                }
    ...
    public static function checkIfCachedSingle($virtuemart_product_id, $front = null, $quantity = 1, $withParent=false, $virtuemart_shoppergroup_ids=0, $prices = true)
    {
    ...
    public $withRating = false;
    public static $_productsSingle = array();
        ...
        if (!empty($this->_id)) {
            ...
            if (!empty($product->categories) and is_array($product->categories)) {
                ...
                if (empty($product->virtuemart_category_id)) {
                    ...
                    } elseif (!empty($product->canonCatId)) {
                    ...
                    } elseif (!empty($product->categories) and is_array($product->categories) and array_key_exists(0, $product->categories)) {
        ...
        if (isset($queryArray[1])) {
            ...
            if ($pos) {
                ...
                if (isset($product->$orderByNameMain)) {
                    ...
                    } elseif (in_array($orderByNameMain, $tableLangKeys)) {
    ...
    */
    public function saveorder_old($cid = array(), $order, $filter = null)
    {
        vRequest::vmCheckToken();
...
//
    ...
    public function lowStockWarningEmail($virtuemart_product_id)
    {
    ...
    public function getUncategorizedChildren($withParent)
    {
        if (!isset($this->_uncategorizedChildren[$this->_id])) {
            ...
            } elseif ($app->isSite() && !VmConfig::get('use_as_catalog', 0) && VmConfig::get('stockhandle', 'none') == 'disableit') {
    ...
    public function getProductChilds($product_id)
    {
        if (empty($product_id)) {
            return array();
        }
    ...
    public function getProductChildIds($product_id)
    {
        if (empty($product_id)) {
            return array();
        }
    ...
    public static function getProductParentId($product_id)
    {
        if (empty($product_id)) {
            return 0;
        }
        ...
        if (!isset($parentCache[$product_id])) {
            ...
            if ($checkedProductKey[0]) {
                ...
                } elseif (isset(self::$_productsSingle[$checkedProductKey[1]]->product_parent_id)) {
     ...
     * @author Max Milbers
     */
    public static function addProductToRecent($productId)
    {
        $session = JFactory::getSession();
     ...
     * @author Max Milbers
     */
    public static function getRecentProductIds($nbr = 3)
    {
        $session = JFactory::getSession();
