...
<table class="html-email" width="100%" cellspacing="0" cellpadding="5" border="0" style="border-collapse: collapse; margin: 0 auto;<?php echo $this->isMail ? ' font-family: Arial, Helvetica, sans-serif; font-size: 12px;' : ''; ?>">
    <tr style="text-align: left;" class="sectiontableheader">
        <th align="left" bgcolor="#EEEEEE" style="border: 1px solid #CCCCCC; display: none;"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_SKU') ?></th>
        ...
        <?php if ($this->doctype == 'invoice') { ?>
            <?php if (VmConfig::get('show_tax')) { ?>
                <th align="center" bgcolor="#EEEEEE" style="border: 1px solid #CCCCCC;">
                    <?php
            ...
            <th align="center" bgcolor="#EEEEEE" style="border: 1px solid #CCCCCC; display: none;"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_SUBTOTAL_DISCOUNT_AMOUNT') ?></th>
    ...
    foreach ($this->orderDetails['items'] as $item) {
        ...
        if ($this->print and !$this->isPdf) {
            $product_name = $item->order_item_name . '&nbsp;<small>' . $item->product_s_desc . '</small>';
        } else {
            $product_name = '<a href="' . JURI::root() . 'index.php?option=com_virtuemart&view=productdetails&virtuemart_category_id=' . $item->virtuemart_category_id .
                '&virtuemart_product_id=' . $item->virtuemart_product_id . '&Itemid=' . $menuItemID . '">' . $item->order_item_name . ' <small>' . $item->product_s_desc . '</small>' . '</a>';
        }

        ?>
        <tr style="vertical-align: top;">
            <td align="left" style="border: 1px solid #CCCCCC; display: none;">
            ...
            <?php if ($this->doctype == 'invoice') { ?>
                ...
                <td align="right" style="border: 1px solid #CCCCCC;  display: none;" class="priceCol">
                ...
                <td align="right" style="border: 1px solid #CCCCCC;" class="priceCol">
                    ...
                    if (!empty($item->product_basePriceWithTax) && $item->product_basePriceWithTax != $item->product_final_price) {
                        echo '<span style="text-decoration: line-through;"> ' . $this->currency->priceDisplay($item->product_basePriceWithT, $this->user_currency_, $qtt)  . '</span><br />';
                    } elseif (empty($item->product_basePriceWithTax) && $item->product_item_price != $item->product_final_price) {
                        echo '<span style="text-decoration: line-through;">' . $this->currency->priceDisplay($item->product_item_price, $this->user_currency_, $qtt) . '</span><br />';
                    }
    ...
    <?php if ($this->doctype == 'invoice') { ?>
        <tr class="sectiontableentry1">
            ...
            <td align="right" style="border: 1px solid #CCCCCC; display: none;"><?php echo "<span class='priceColor2'> "  . $this->currency->priceDisplay($this->orderDetails['details']['BT']->order_discountAmount,  $this->user_currency_id) . "</span>" ?></td>
        ...
        <?php
        if (false && $this->orderDetails['details']['BT']->coupon_discount <> 0.00) {
            $coupon_code = $this->orderDetails['details']['BT']->coupon_code ? '  ( ' . $this->orderDetail['details']['BT']->coupon_code . ')'  : '';
            ?>
            <tr>
                <td align="right" style="border: 1px solid #CCCCCC;" class="pricePad" colspan="6">
                    <?php echo vmText::_('COM _VIRTUEMART_COUPON_DISCOUNT') . $coupon_code ?>
                </td>
                <?php if (VmConfig::get('show_tax')) { ?>
                ...
                <td align="right" style="border: 1px solid #CCCCCC;">&nbsp;</td>
                <td align="right" style="border: 1px solid #CCCCCC;"><?php echo $this->currency->priceDisplay($this->orderDetails['detais']['BT']->coupon_discount, $this->user_currency_id); ?></td>
            </tr>
        <?php
        ...
        if (false && $discountsBill)
            ...
            <tr>
                ...
                <td align="right" style="border: 1px solid #CCCCCC;">
                    <?php echo $this->currency->priceDisplay($rule->calc_amount, $this->user_currency_id); ?>
                </td>
                <td align="right" style="border: 1px solid #CCCCCC;">
                    <?php echo $this->currency->priceDisplay($rule->calc_amount, $this->user_currency_id); ?>
                </td>
    ...
    <tr>
        ...
        <td align="right" style="border: 1px solid #CCCCCC; display: none;">&nbsp;</td>
    ...
    <tr>
        ...
        <td align="right" style="border: 1px solid #CCCCCC; display: none;"><span class='priceColor2'><?php echo $this->currency->priceDisplay($this->orderDetails['details']['BT']->order_billDiscountAmount, $this->user_currency_id); ?></span></td>
    ...
    if (
        !empty($this->orderDetails['details']['BT']->payment_currency_rate)
        ...
        <tr>
            ...
            <td align="right" style="border: 1px solid #CCCCCC;" class="pricePad" colspan="<?php echo $colspan_pcr ?>">
                <?php
